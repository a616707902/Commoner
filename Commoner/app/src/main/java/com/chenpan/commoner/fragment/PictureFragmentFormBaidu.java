...
import com.chenpan.commoner.mvp.presenter.PicturePresenterBaidu;
 ...
 */
public class PictureFragmentFormBaidu extends BaseFragment<IPictureView, PicturePresenterBaidu> implements IPictureView {
    ...
    @Override
    public PicturePresenterBaidu createPresenter() {
        return new PicturePresenterBaidu();
    }
    ...
    public void bindViewAndAction(Bundle savedInstanceState) {
        ...
        if (TextUtils.isEmpty(type) || mPresenter == null || !(mPresenter instanceof PicturePresenterBaidu)) {
            return;
        }
        flLoading.withLoadedEmptyText("≥﹏≤ , 连条毛都没有 !").withEmptyIco(R.drawable.note_empty).withBtnEmptyEnnable(false)
            ...
            @Override
            public void onRetry() {
                mPresenter.getPicture(getActivity(),  type, params);
            }
        }).build();
        ...
        recyclerView.addOnScrollListener(new RecyclerView.OnScrollListener() {
            ...
            public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
                ...
                if (canLoadMore) ;
                PictureFragmentFormBaidu.this.onScrolled(recyclerView, dx, dy);
            }
        });
    ...
    public void setAdapterBaidu(List<PictureBeanBaiDu> list) {
        ...
        if (pageNo < pageSize)
            canLoadMore = false;
        if (mAdapter == null) {
            mAdapter = new BaseRecyclerAdapter(list, R.layout.list_item_images_list, PictureBaiduHolder.class);
            recyclerView.setAdapter(mAdapter);
        } else {
            if ((mAdapter.getItem(0) == null) && (list.size() == 0))
                return;
            if ((mAdapter.getItem(0) == null) || (list.size() == 0) || (!((PictureBeanBaiDu) mAdapter.getItem(0)).getThumbnailUrl().equals(((PictureBeanBaiDu) list.get(0)).getThumbnailUrl())))
